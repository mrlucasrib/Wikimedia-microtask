...
local I18n = 'BrewerColors'

--[[ 
TRANSLATION, in :Commons:Data:I18n/BrewerColors.tab
local RS_ColorNumInvalid = 'El nombre de colors ($1) està fora del rang'
local RS_ColorNameInvalid = 'El nom del color ($1) no és vàlid'
--]]
local RS_ColorNumInvalid = 'ColorNumInvalid'
local RS_ColorNameInvalid = 'ColorNameInvalid'

local function I18nStr (S, ...)
	return TNTT.GetMsgP (I18n, S, {...})
end

-- This product includes ColorName specifications and designs developed by Cynthia Brewer (http://colorbrewer.org/).
local ColorYlGn = {
...
function p.GetColors(ColorName, IsInv, N, CallError)
	local ColorFound = {}	
	function GetC(ColorSet)
		if (N < 3) or (N > (table.getn(ColorSet)+2)) then
			if CallError then
				error (I18nStr(RS_ColorNumInvalid,tostring(N)), 0)
			end	
		elseif IsInv then
			for i=table.getn(ColorSet[N-2]), 1, -1  do
				table.insert (ColorFound, '#'..ColorSet[N-2][i])
			end  
		else	
			for i=1, table.getn(ColorSet[N-2]) do
				table.insert (ColorFound, '#'..ColorSet[N-2][i])
			end  
		end	
	end	
	local ColorNameL = string.lower (ColorName)
	if     ColorNameL == 'ylgn'		then GetC(ColorYlGn)
	elseif ColorNameL == 'ylgnbu'	then GetC(ColorYlGnBu)
	elseif ColorNameL == 'gnbu'		then GetC(ColorGnBu)
	elseif ColorNameL == 'bugn'		then GetC(ColorBuGn)
	elseif ColorNameL == 'pubugn'	then GetC(ColorPuBuGn)
	elseif ColorNameL == 'pubu'		then GetC(ColorPuBu)
	elseif ColorNameL == 'bupu'		then GetC(ColorBuPu)
	elseif ColorNameL == 'rdpu'		then GetC(ColorRdPu)
	elseif ColorNameL == 'purd'		then GetC(ColorPuRd)
	elseif ColorNameL == 'orrd'		then GetC(ColorOrRd)
	elseif ColorNameL == 'ylorrd'	then GetC(ColorYlOrRd)
	elseif ColorNameL == 'ylorbr'	then GetC(ColorYlOrBr)
	elseif ColorNameL == 'purples'	then GetC(ColorPurples)
	elseif ColorNameL == 'blues' 	then GetC(ColorBlues)
	elseif ColorNameL == 'greens'	then GetC(ColorGreens)
	elseif ColorNameL == 'oranges'	then GetC(ColorOranges)
	elseif ColorNameL == 'reds'		then GetC(ColorReds)
	elseif ColorNameL == 'greys' 	then GetC(ColorGreys)
	elseif ColorNameL == 'puor'		then GetC(ColorPuOr)
	elseif ColorNameL == 'brbg'		then GetC(ColorBrBG)
	elseif ColorNameL == 'prgn'		then GetC(ColorPRGn)
	elseif ColorNameL == 'piyg'		then GetC(ColorPiYG)
	elseif ColorNameL == 'rdbu'		then GetC(ColorRdBu)
	elseif ColorNameL == 'rdgy'		then GetC(ColorRdGy)
	elseif ColorNameL == 'rdylbu'	then GetC(ColorRdYlBu)
	elseif ColorNameL == 'spectral'	then GetC(ColorSpectral)
	elseif ColorNameL == 'rdylgn'	then GetC(ColorRdYlGn)
	elseif ColorNameL == 'accent'	then GetC(ColorAccent)
	elseif ColorNameL == 'dark2' 	then GetC(ColorDark2)
	elseif ColorNameL == 'paired'	then GetC(ColorPaired)
	elseif ColorNameL == 'pastel1'	then GetC(ColorPastel1)
	elseif ColorNameL == 'pastel2'	then GetC(ColorPastel2)
	elseif ColorNameL == 'set1'		then GetC(ColorSet1)
	elseif ColorNameL == 'set2'		then GetC(ColorSet2)
	elseif ColorNameL == 'set3'		then GetC(ColorSet3)
	else
		if CallError then
...
function p.ShowColors(frame)
	...
	local ColorFound = {}
	ColorFound = p.GetColors (ColorName, IsInv, N, true)
	local boxes = {}
