...
function catlinkfollowr( catname, displaytext )
	...
	if listall then
		if nilorR then --#R followed
			table.insert( listofalllinks, '[[:Category:'..catname..']] → '..'[[:Category:'..nilorR..']] ('..link..')' )
		else --no #R
			table.insert( listofalllinks, '[[:Category:'..catname..']] ('..link..')' )
		end
	end
...
function nav_hyphen( start, hyph, finish, firstpart, lastpart, minseas, maxseas, testgap )
	...
	--	start     = 2015
	--	hyph      = –
	--	finish    = 16 (sequential years can be abbreviated, but others should be full year, i.e. "2001–2005")
...
function nav_year( firstpart, year, lastpart, minimumyear, maximumyear )
	...
	if string.match(lastpart, '^-') then
		tspace = '' --i.e. Category:2018-related timelines
	end
...
function p.find_var( pn )
	...
	local d_season = mw.ustring.match(cpagename, ':(%d+s).+%(%d+[–-]%d+%)') --i.e. [[Category:1760s in the Province of Quebec (1763–1791)]]
...
function p.navseasoncats( frame )
	...
	if skip_gaps and skip_gaps == 'yes' then
		skipgaps = true
		if avoidself then
			misctrackingcats[15] = '[['..testcasecolon..'Category:Navseasoncats using skip-gaps parameter]]'
		end
	end
