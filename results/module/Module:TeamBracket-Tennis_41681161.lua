...
local function parseArgs(frame)
	...
	if (args['byes'] and (args['byes'] == 'yes' or args['byes'] == 'y')) then
		tcats = tcats .. '[[Category:Pages using a tennis bracket with deprecated syntax|B]]'
	end
end
...
local function parseSets(s)
	if not s then tcats = tcats .. '[[Category:Pages using a tennis bracket with the default number of sets]]' end
	...
	for r=1,n do
		...
		elseif sets[r-1] then
			...
			sets[r] = 5
			tcats = tcats .. '[[Category:Pages using a tennis bracket with the default number of sets]]'
		end
	end
end
...
local function getTeamArg(round, type, team)
	...
	if mw.ustring.find(value, '[%s]*<[%s/]*[Bb][Rr][%s/]*>[%s ]*&[Nn][Bb][Ss][Pp];[%s]*') then
		tcats = '[[Category:Pages using a team bracket with nbsp]]'
	end
