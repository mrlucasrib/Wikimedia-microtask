-- This module implements [[Template:Single ratings]].
...
function p._main(args)
	...
	if mw.title.getCurrentTitle().namespace == 0 and yesno(args.noprose) then
		root:wikitext('[[Category:Articles with single ratings that need to be turned into prose]]')
	end
	if duplicateScores then
		root:wikitext('[[Category:Pages using single ratings with duplicate score parameters]]')
	end
...
function p.main(frame)
	local args = require('Module:Arguments').getArgs(frame, {
		wrappers = 'Template:Single ratings'
	})
	return p._main(args)
end
